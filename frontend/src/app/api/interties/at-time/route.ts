  ...
  try {
    ...
    if (requestedTime > now) {
      ...
      try {
        ...
        console.warn('ClickHouse intertie at-time query failed (future):', e);
      }

      return NextResponse.json({
        data,
        timestamp,
        source: data.length > 0 ? 'scheduled' : 'unavailable',
      }, {
    ...
    try {
      ...
      console.warn('ClickHouse intertie at-time query failed (historical):', e);
    }

    return NextResponse.json({
      data,
      timestamp,
      source: data.length > 0 ? 'actual' : 'unavailable',
    }, {
      headers: { 'Cache-Control': 's-maxage=60, stale-while-revalidate=300' },
    });
  } catch (error) {
